---
title: "PD Bayesian Regression Analysis"
author: "Carl Beuchel"
date: today
...
library(brms)
library(cmdstanr)
cmdstanr::set_cmdstan_path("/home/carl/Dokumente/01_programs/git-repositories/cmdstan/")
cmdstanr::cmdstan_version()
cmdstanr::check_cmdstan_toolchain()
setOption("brms.backend", "cmdstanr")
setOption("mc.cores", 4)
getOption("brms.backend")
getOption("mc.cores")


model_formula_20 <- paste0(
  "Testing20 ~ 1 + (1 + ", 
...
model_formula_20 <- "Testing20 ~ 1 + (1 + VE + GI + CHE + UP + log_PD + EP + Gini || GBDR7_f)"

brm_20 <- brms::brm(formula = model_formula_20,
...
launch_shinystan(brm_20)
brms::posterior_interval(brm_20, prob = 0.9)
brms::predictive_error()
brms::predictive_interval(brm_20)

posterior::rhat(brm_20)
bayesplot::mcmc_dens(brm_20)
...
mod_negbin_20 <- stan_glm(formula = model_formula_20, family = neg_binomial_2(link = "log"), data = data_scaled)
mod_negbin_21 <- stan_glm(formula = model_formula_21, family = neg_binomial_2(link = "log"), data = data_scaled)

# Summary of each model to 
mod_negbin_21  |> broom.mixed::tidy(conf.int = TRUE, exponentiate = TRUE) |> kable()
mod_negbin_20  |> broom.mixed::tidy(conf.int = TRUE, exponentiate = TRUE) |> kable()

# Save the coefficients
mod_negbin_20_coefs <- cbind(
  mod_negbin_20 |> tidy() |> as.data.table(),
  as.data.frame(posterior_interval(mod_negbin_20))[-14, ]
)
mod_negbin_21_coefs <- cbind(
  mod_negbin_21 |> tidy() |> as.data.table(),
  as.data.frame(posterior_interval(mod_negbin_21))[-14, ]
)

# join data for plotting
mod_negbin_20_coefs[, year := "2020"]
mod_negbin_21_coefs[, year := "2021"]
mod_negbin_coefs <- rbindlist(list(mod_negbin_20_coefs, mod_negbin_21_coefs))
...
mod_negbin_coefs[, `:=`(rel_change  = exp_estimate - 1, 
                        rel_change_5 = exp_5 - 1, 
                        rel_change_95 = exp_95 - 1)]
...
Plot the results. Reference group for the Region effects is "Central Europe,
Eastern Europe, and Central Asia". Displayed effects are independent of region
...
# Custom forest plot
ggplot(mod_negbin_coefs[term != "(Intercept)"],
       aes(color = year,
           y = term)
           ) +
  ...
  geom_linerange(aes(xmin = rel_change_5 * 100, 
                     xmax = rel_change_95 * 100,
                     group = year),
                 position = position_dodge2(width = 0.8, reverse = TRUE)
                 ) + 
...
for (test_plan_i in test_plan$run_index) {
  ...
  # Create posterior predictions and calculate intervals
  
  for(i in ncol(post_predictions)) {
    hdi(post_predictions[, i])
  }

  
  tmp <- posterior_linpred(model_list[[my_year]], newdata = new_dat)
  tmp <- posterior_epred(model_list[[my_year]], newdata = new_dat)
  tmp <- posterior_predict(model_list[[my_year]], newdata = new_dat)
  tmp <- posterior_interval(model_list[[my_year]])
  predict(model_list[[my_year]], newdata = new_dat, se.fit = TRUE)
  tmp[,1] |>  hdi()
  tmp[,180] |> log() |> hist()
  ...
  new_dat_pi <- predictive_interval(
    object = model_list[[my_year]], 
    newdata = new_dat) |> 
    as.data.frame()
  tmp <- cbind(new_dat, new_dat_pi)
  
  ggplot(tmp, aes(x = CHE, ymin = `5%`, ymax = `95%`)) + 
    geom_ribbon(fill = "royalblue1", alpha = 0.3) + 
    facet_grid(~GBDR7_f) +
    scale_y_continuous(
    trans = "log10",
...
```{r}
ggplot(res_ci_pi[Year == "2021"], aes(x = index_range, y = ci_pred))  + 
  facet_grid(
    pred_index~GBDR7_f, 
    labeller = labeller(
      .rows =  c(CHE = "CHE", UP = "UP", 
...
# Custom forest plot
ggplot(results_regional[Predictors != "Intercept"],
       aes(color = Year,
           y = Predictors)
           ) +
...
## Global Vaccination (2021)/Mortality (2020/2021) Regression 
...
## Compare Vaccination MMR vs. Covid
...
# Custom forest plot
ggplot(mod_negbin_vacc_coefs[term != "(Intercept)"],
...
# Model fit (including dispersion parameter theta estimation)
mod_mortality_20 <- glm.nb(formula = model_mortality_20, 
                           data = data_vacc, 
                           control = glm.control(
                             maxit = 30,
                             trace = TRUE
...
# Summary of each model to 
confint(mod_mortality_20, parm = "VE")
profile(mod_mortality_20)

mod_mortality_20  |> tidy(conf.int = TRUE, exponentiate = TRUE) |> kable()
mod_mortality_21  |> tidy(conf.int = TRUE, exponentiate = TRUE) |> kable()

# Save the coefficients
mod_mortality_20_coefs <- cbind(
  mod_mortality_20 |> tidy() |> as.data.table(),
  confint(mod_mortality_20)
)
